...
their ability to use certain keying material (tied to the
authenticated identity) in the DTLS handshake.  The authorization
server assists in this process by incorporating keying material (or
information about keying material) into the access token, which is
considered a "proof of possession" token.
